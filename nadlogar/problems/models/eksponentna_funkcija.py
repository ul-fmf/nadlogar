...
class GrafEksponentne(Problem):
    ...
    def generate(self):
        x = sympy.symbols('x')
        osnova, premik, eksponentna_prva = naredi_eksponentno(premik = random.choice([i for i in range(-3, 4) if i != 0]), cela_osnova=True)
        predznak = random.choice([1, -1])
        premik_drugi = random.choice([x for x in range(-3, 4) if x != 0])
        eksponentna_druga = sympy.Add(predznak * sympy.Pow(osnova, x, evaluate=False), premik_drugi, evaluate=False)
        narisi_eksponentna_prva = str(eksponentna_prva).replace('**', '^')
        narisi_eksponentna_druga = str(eksponentna_druga).replace('**', '^')
        return {'eksponentna_prva': sympy.latex(eksponentna_prva), 'eksponentna_druga': sympy.latex(eksponentna_druga), 'narisi_eksponentna_prva': sympy.latex(narisi_eksponentna_prva),
                'narisi_eksponentna_druga': sympy.latex(narisi_eksponentna_druga), 'premik_drugi': sympy.latex(premik_drugi)}
...
class ResevanjeEksponentneEnacbe(Problem):
    ...
    def generate(self):
        ...
        vrednost = sympy.Rational(osnova ** (resitev + zamik_clena1) + k_clena2 * (osnova) ** (resitev + zamik_clena2))
        enacba = sympy.Eq(sympy.Pow(osnova, (x + zamik_clena1)) + k_clena2 * sympy.Pow(osnova, (x + zamik_clena2)), vrednost)
...
class ResevanjeEksponentneEnacbeZDvemaOsnovama(Problem):
    ...
    def generate(self):
        ...
        k_osnove2_levi = osnova2 ** u - osnova1 ** komponenta2_zamika_enacbe1 * k_osnove1_levi
        k_osnove2_desni = osnova1 ** v - osnova2 ** komponenta2_zamika_enacbe2 * k_osnove1_desni
        resitev = v - komponenta1_zamika_enacbe1
        enacba = sympy.Eq(sympy.simplify(k_osnove1_levi * osnova1 ** (x + komponenta1_zamika_enacbe1 + komponenta2_zamika_enacbe1) - k_osnove1_desni * osnova2 ** (x + komponenta1_zamika_enacbe2 + komponenta2_zamika_enacbe2), rational = True),
                          sympy.simplify(-k_osnove2_levi * osnova1 ** (x + komponenta1_zamika_enacbe1) + k_osnove2_desni * osnova2 ** (x + komponenta1_zamika_enacbe2), rational = True))
