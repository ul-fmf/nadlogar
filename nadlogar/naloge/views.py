...
def podrobnosti(request, id: int):
    ...
    return render(request, 'naloge/podrobnosti.html', {'naloga': naloga, 'besedilo': besedilo, 'resitev': resitev})
...
def izberi_tip(request, test_id: int):
    ...
    return render(request, 'naloge/izberi_tip.html', {'content_types': content_types, 'test': test})


def dodaj(request, test_id: int, content_type_id: int):
    test = get_object_or_404(Test, id=test_id)
    content_type = ContentType.objects.get_for_id(content_type_id)
    naloga_form = NalogaForm.obrazec_modela(content_type.model_class())
...
def uredi(request, id: int):
    naloga = get_object_or_404(Naloga, id=id).doloci_tip()
    naloga_form = NalogaForm.obrazec_naloge(naloga)
